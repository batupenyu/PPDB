...
// Try WASM fallback first, then native binding
let nativeExport;
try {
	// Attempt to load WASM-based implementation (no native dependencies)
	nativeExport = require('@rollup/wasm-node/dist/native.js');
} catch (wasmError) {
	try {
		// Fallback to native binding
		...
		nativeExport = requireWithFriendlyError(
			existsSync(path.join(__dirname, localName)) ? localName : `@rollup/rollup-${packageBase}`
		);
	} catch (finalError) {
		// If both fail, provide stub that throws when used
		nativeExport = {
			parse: () => { throw new Error('Rollup native binding unavailable. Install @rollup/wasm-node or fix native module issue.'); },
			parseAsync: () => { throw new Error('Rollup native binding unavailable. Install @rollup/wasm-node or fix native module issue.'); },
			xxhashBase64Url: () => { throw new Error('Rollup native binding unavailable. Install @rollup/wasm-node or fix native module issue.'); },
			xxhashBase36: () => { throw new Error('Rollup native binding unavailable. Install @rollup/wasm-node or fix native module issue.'); },
			xxhashBase16: () => { throw new Error('Rollup native binding unavailable. Install @rollup/wasm-node or fix native module issue.'); }
		};
	}
}

const { parse, parseAsync, xxhashBase64Url, xxhashBase36, xxhashBase16 } = nativeExport;
